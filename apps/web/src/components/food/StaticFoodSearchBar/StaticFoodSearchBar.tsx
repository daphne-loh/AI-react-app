...
interface StaticFoodSearchBarProps {
  onFiltersUpdate: (searchTerm: string, filters: Omit<FoodSearchFilters, 'searchTerm'>) => void;
  onReset: () => void;
  loading?: boolean;
  className?: string;
}
...
export const StaticFoodSearchBar: React.FC<StaticFoodSearchBarProps> = ({
  onFiltersUpdate,
  onReset,
  loading = false,
  className = ''
  ...
  const updateFilters = () => {
    const filters: Omit<FoodSearchFilters, 'searchTerm'> = {};
    ...
    onFiltersUpdate(searchTerm, filters);
  };

  const handleSearchChange = (value: string) => {
    setSearchTerm(value);
    // Update immediately for search
    const filters: Omit<FoodSearchFilters, 'searchTerm'> = {};
    if (selectedTheme) filters.theme = selectedTheme as ThemeCategory;
    if (selectedRarity) filters.rarity = selectedRarity as 'common' | 'uncommon' | 'rare' | 'legendary';
    if (selectedTags.length > 0) filters.tags = selectedTags;
    
    onFiltersUpdate(value, filters);
  };
